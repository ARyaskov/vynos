...
const CONTAINER_ID = "vynos-wallet-frame-container"
...
function closeElement(document: HTMLDocument, baseUrl: string) {
  ...
  el.src = new URL("styles/images/up-arrow.svg", baseUrl).toString()
...
export default class Frame {
  ...
  async containerElement(): Promise<HTMLDivElement> {
    if (!this._containerElement) {
      ...
      const style =
        "#vynos_frame_img_close_button{width: 40px;bottom: 3px;position: absolute;left: 50%;margin-left: -20px;opacity:0;transition: opacity 1s}" +
