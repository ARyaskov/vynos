...
vynos.ready().then((wallet) => {
  wallet.provider.request({ method: "eth_accounts" }).then((accounts) => {
...
- `wallet.openChannel(receiverAccount: string, channelValue: bigint): Promise<PaymentChannel>` - open a payment channel,
...
Use `pnpm` package manager with Node.js 24+.

## Install

```
git clone <your-vynos-repository-url>
cd vynos
pnpm install
```
...
That command starts a Vite development server on localhost.
Open browser on `http://localhost:5173/harness/index.html` and click on some buttons.
...
It demonstrates Vynos work on localhost as a local test bench for wallet flows.
